...
create or replace transient table OLIST_DW.MARTS.dim_customers
    
    
    
    as (-- models/marts/dim_customers.sql
-- Customer dimension enriched with LTV, order aggregates, and Brazilian region

WITH customers AS (
    SELECT * FROM OLIST_DW.STAGING.stg_customers
),
...
states AS (
    SELECT * FROM OLIST_DW.MARTS.brazil_states
)
